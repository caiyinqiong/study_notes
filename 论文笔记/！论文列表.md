...
- [x] （过于简单，不合适）RankQA: Neural Question Answering with Answer Re-Ranking 

- [x] （苏）Episodic Memory Reader: Learning What to Remember for Question Answering from Streaming Data 

- [x]  （蔡）Retrieve, Read, Rerank: Towards End-to-End Multi-Document Reading Comprehension

- [x] （过于简单，不合适）Latent Retrieval for Weakly Supervised Open Domain Question Answering

- [x] （蔡）Multi-Hop Paragraph Retrieval for Open-Domain Question Answering

- [x] （有点过于简单）Real-Time Open-Domain Question Answering with Dense-Sparse Phrase Index

- [x] A cross-sentence latent variable model for semi-supervised sequence matching

- [x] （苏）Answering while Summarizing: Multi-task Learning for Multi-hop QA with Evidence Extraction 

- [x] （蔡）Learning a Matching Model with Co-teaching for Multi-turn Response Selection in Retrieval-based Dialogue Systems

- [x] （有点过于简单）Matching Article Pairs with Graphical Decomposition and Convolutions

- [x] （陈）RE2: Simple and Effective Text Matching with Richer Alignment Features

  

- [x] （不合适）A Lightweight Recurrent Network for Sequence Modeling

- [x] （过于简单）Encouraging Paragraph Embeddings to Remember Sentence Identity Improves Classification

- [x] （蔡）Pretraining Methods for Dialog Context Representation Learning

- [x] （不合适）Relational Word Embeddings

- [x] （过于简单）Training Neural Response Selection for Task-Oriented Dialogue Systems

- [x] （蔡）Learning Transferable Feature Representations Using Neural Networks

  

  

- [ ] （苏）Entity-Relation Extraction as Multi-Turn Question Answering
- [ ] （苏）Multi-hop reading comprehension across multiple documents by reasoning over heterogeneous graphs
- [ ] （苏）Multi-style Generative Reading Comprehension
- [ ] 
...
- [ ] ！！！（对话系统）Global-to-local Memory Pointer Networks for Task-Oriented Dialogue

- [ ] ！！！（对话系统）Large-Scale Answerer in Questioner's Mind for Visual Dialog Question Generation

- [ ] ！！！（对话系统）Wizard of Wikipedia: Knowledge-Powered Conversational Agents

- [ ] ！！！（对话生成）Detecting Egregious Responses in Neural Sequence-to-sequence Models 

- [ ] ！！！（信息检索）textTOvec: DEEP CONTEXTUALIZED NEURAL AUTOREGRESSIVE TOPIC MODELS OF LANGUAGE WITH DISTRIBUTED COMPOSITIONAL PRIOR

- [ ] ！！！（NLU）GLUE: A Multi-Task Benchmark and Analysis Platform for Natural Language Understanding 

- [ ] ！！！（QA）Coarse-grain Fine-grain Coattention Network for Multi-evidence Question Answering

- [ ] ！！！（QA）Multi-step Retriever-Reader Interaction for Scalable Open-domain Question Answering

- [x] ！！！（QA）Generative Question Answering: Learning to Answer the Whole Question
